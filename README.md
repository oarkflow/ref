...
- [**docs/identity-and-signing.md**](docs/identity-and-signing.md): The built-in user directory (`identity.users`): argon2id sign-in that issues tenant-scoped JWTs, invitations, memberships with roles and org units, suspension, last-admin protection and an audit trail. Also asymmetric signing keys (`crypto.signer`: Ed25519, RSA-PSS, RSA-PKCS1v15) with key ids, rotation and a JWKS endpoint, used for EdDSA/RS256 tokens, offline-verifiable certificates, signed deploy revisions and arbitrary payloads. The worked example is [examples/identity](examples/identity).
