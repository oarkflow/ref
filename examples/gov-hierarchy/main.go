// Command gov-hierarchy runs the gov-hierarchy example application.
//
//	go run ./examples/gov-hierarchy                     # serve on :8095
//	go run ./examples/gov-hierarchy -token '{"sub":"u1","roles":["admin"]}'
//
...
func main() {
	config := flag.String("config", "examples/gov-hierarchy/app.bcl", "path to the application document")
	...
	if err != nil {
		log.Fatalf("gov-hierarchy: compile: %v", err)
	}
	defer app.Close()
	...
	if err := app.Mount(server); err != nil {
		log.Fatalf("gov-hierarchy: mount: %v", err)
	}
	for _, route := range app.Document.Routes {
		log.Printf("gov-hierarchy: %-6s %s -> %s", route.Method, route.Path, route.Intent)
	}
	...
	log.Printf("gov-hierarchy: listening on %s", *addr)
	if err := server.Listen(*addr); err != nil && !errors.Is(err, http.ErrServerClosed) {
		log.Fatalf("gov-hierarchy: %v", err)
	}
}
